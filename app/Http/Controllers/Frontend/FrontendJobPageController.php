...
class FrontendJobPageController extends Controller
{
    function index(Request $request): View
    {
        ...
        return view('frontend.pages.job-index', compact('jobs', 'countries', 'jobCategories', 'jobTypes', 'selectedStates', 'selectedCites'));
    }
    ...
    function show(string $slug): View
    {
        ...
        $alreadyApplied = AppliedJob::where(['job_id' => $job->id, 'candidate_id' => auth()->user()->candidate?->id])->exists();
    ...
    function applyJob(string $id): Response
    {
        ...
        if (auth()->check() && auth()->user()->role !== 'candidate') {
            throw ValidationException::withMessages(['only candidate will able to Applied For Job']);
        }
        ...
        $alreadyApplied = AppliedJob::where(['job_id' => $id, 'candidate_id' => auth()->user()->candidate?->id])->exists();
        if ($alreadyApplied) {
        ...
        $applyJob->job_id = $id;
        $applyJob->candidate_id = auth()->user()->candidate->id;
        $applyJob->save();
