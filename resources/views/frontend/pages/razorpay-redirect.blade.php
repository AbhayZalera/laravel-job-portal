...
<body>
    @php
        $payableAmount = session('selected_plan')['price'] * config('gatewaySettings.razorpay_currency_rate') * 100;
    @endphp
    <form action="{{ route('company.razorpay.payment') }}" method="POST" id="razorpay-form">
        @csrf
        <input type="hidden" name="payment_abandoned" id="payment_abandoned" value="0">
        <script src="https://checkout.razorpay.com/v1/checkout.js" data-key="{{ config('gatewaySettings.razorpay_key') }}"
            data-currency="{{ config('gatewaySettings.razorpay_currency_name') }}" data-amount="{{ $payableAmount }}"
            data-buttontext="Pay" data-name="{{ session('selected_plan')['label'] . ' Plan' }}"
            data-description="payment for the product" data-theme.color="#1ca774" data-modal.ondismiss="handleDismiss"></script>
    </form>
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            var button = document.querySelector(".razorpay-payment-button");
